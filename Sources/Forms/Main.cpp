...
// ---------------------------------------------------------------------------
SysProcInfo SysProcs[] = { { "@HandleFinally", 0 },
...
int CodeHistoryMax;                       // Max pointer position of Code navigation History Array
										  // (for ->)
...
void __fastcall TFMain_11011981::StrapVMT(int pos, int ConstId, MConstInfo* ConstInfo)
{
	...
	for(int n = 0; n < ConstInfo->FixupNum; n++) {
		...
		if(VMTOffset == cVmtDynamicTable) {
			// Ïðîïóñêàåì, ïîñêîëüêó äèíàìè÷åñêèå âûçîâû áóäóò îáðàáîòàíû ñðåäè ïðî÷èõ
			// ôèêñàïîâ
			continue;
		}
		...
		if(VMTOffset == cVmtParent) {
			// Óêàçûâàåò íà ðîäèòåëüñêèé êëàññ, íå îáðàáàòûâàåì, ïîñêîëüêó îí
			// âñå-ðàâíî âñòðåòèòñÿ îòäåëüíî
			continue;
		}
					...
					// Else may be partial unmatching
						...
						if(!SameText(name, fName)) {
							String name1 = name.SubString(1,
														  name.Length() - 1); // Trim last symbol ('A','W') - GetWindowLongW(A)
		...
		if(!recN->MakeArgsManually()) {
			...
			if(p) {
				for(int k = 0; k < ProcInfo->ArgsNum; k++) {
					...
					if(SameText(argInfo.TypeDef, "Int64") || SameText(argInfo.TypeDef, "Real") || SameText(argInfo.TypeDef, "Real48") ||
					   SameText(argInfo.TypeDef, "Comp") || SameText(argInfo.TypeDef, "Double") || SameText(argInfo.TypeDef, "Currency") ||
	...
	if(useFixups && ProcInfo->FixupNum) {
		...
		for(int n = 0; n < ProcInfo->FixupNum; n++) {
			...
			if(SameText(fName, ProcInfo->ProcName)) {
				...
				// CaseStudio, 405ae4 - call [offs+4*eax] - how to use offs? And offs
				// has cfLoc Val inside procedure - possible jump address for switch (or
				// call)
				if(fixupInfo.Type == 'J') {
...
int __fastcall TFMain_11011981::GetDelphiVersion()
{
	...
	for(int n = 0; n < TotalSize - 14; n += 4) {
		if(Image[n] == 7 && Image[n + 1] == 8 && Image[n + 2] == 'T' && Image[n + 3] == 'C' && Image[n + 4] == 'o' && Image[n + 5] == 'n' &&
		   Image[n + 6] == 't' && Image[n + 7] == 'r' && Image[n + 8] == 'o' && Image[n + 9] == 'l') {
			...
			if(IsValidImageAdr(vmtAdr)) {
				...
				if(TControlInstSize == 0x15C) // 6 èëè 7
				{
					...
					for(int m = 0; m < TotalSize - 11; m += 4) {
						if(Image[m] == 7 && Image[m + 1] == 5 && Image[m + 2] == 'T' && Image[m + 3] == 'F' && Image[m + 4] == 'o' && Image[m + 5] == 'r' &&
						   Image[m + 6] == 'm') {
							// Ïîñëå òèïà äîëæåí ñëåäîâàòü óêàçàòåëü íà òàáëèöó VMT (0)
	...
	/*
      ...
      for (int n = 0; n < num; n++, unitsTab += 8)
      {
              ...
              if (iniAdr && IsValidImageAdr(iniAdr) && IsFlagSet(cfImport,
     Adr2Pos(iniAdr)))
              {
                      recN = GetInfoRec(iniAdr);
                      pos = recN->name.Pos(".");
                      if (pos && SameText(recN->name.SubString(pos + 1, 22),
     "@System@Initialization"))
              ...
              if (finAdr && IsValidImageAdr(finAdr) && IsFlagSet(cfImport,
     Adr2Pos(finAdr))))
              {
		...
		while(1) {
			...
			if(dd == 'hsup' && disInfo.OpType[0] == otIMM && disInfo.Immediate) {
				...
				if(IsValidImageAdr(modTable)) {
					...
					if(endAdr < Base || endAdr >= Base + Size)
						unitsPos = 0;
...
int __fastcall TFMain_11011981::IsValidCode(DWORD fromAdr)
{
	...
	for(row = 0; row < outRows; row++) {
		...
		if(!memcmp(DisInfo.Mnem, "arpl", 4) || !memcmp(DisInfo.Mnem, "out", 3) ||
		...
		if(!row) {
			...
			if(DisInfo.Branch)
				return -1;
			if(!memcmp(DisInfo.Mnem, "bound", 5) || !memcmp(DisInfo.Mnem, "retf", 4) || !memcmp(DisInfo.Mnem, "pop", 3) || !memcmp(DisInfo.Mnem, "aaa", 3) ||
			   !memcmp(DisInfo.Mnem, "adc", 3) || !memcmp(DisInfo.Mnem, "sbb", 3) || !memcmp(DisInfo.Mnem, "rcl", 3) || !memcmp(DisInfo.Mnem, "rcr", 3) ||
			   !memcmp(DisInfo.Mnem, "clc", 3) || !memcmp(DisInfo.Mnem, "stc", 3))
				return -1;
		}
...
Interfaces
        ...
        table of interfaces as part of the class's RTTI. The VMT points to the
table of interfaces, which starts with a 4-byte count, followed by a list of
        interface records. Each interface record contains the GUID, a pointer to
the interface's VMT, the offset to the interface's hidden field, and a pointer
        to a property that implements the interface with the implements
directive. If the offset is zero, the interface property (called ImplGetter)
must be non-nil, and if the offset is not zero, ImplGetter must be nil. The
interface property can be a reference to a field, a virtual method, or a static
method, following the conventions of a property reader (which is described
earlier in this chapter, under Section 3.2.3"). When an object is constructed,
Delphi automatically checks all the interfaces, and for each interface with a
        non-zero IOffset, the field at that offset is set to the interface's
VTable (a pointer to its VMT). Delphi defines the the types for the interface
table, unlike the other RTTI tables, in the System unit. These types are shown
in Example 3.9. Example 3.9. Type Declarations for the Interface Table
...
See for the details of the GetInterface, GetInterfaceEntry, and
GetInterfaceTable methods.
*/
		...
		if(IsValidCodeAdr(vAdr)) {
			...
			for(int v = 0;; v++) {
				...
				while(1) {
					...
					if((disInfo.OpType[0] == otMEM || disInfo.OpType[1] == otMEM) && disInfo.BaseReg != 20) // to exclude instruction "xchg reg, [esp]"
					{
...
/*
Automated Methods
        The automated section of a class declaration is now obsolete because it
is easier to create a COM automation server with Delphi's type library editor,
        using interfaces. Nonetheless, the compiler currently supports automated
        declarations for backward compatibility. A future version of the
compiler might drop support for automated declarations. The OleAuto unit tells
you the details of the automated method table: The table starts with a 2-byte
count, followed by a list of automation records. Each record has a 4-byte dispid
(dispatch identifier), a pointer to a short string method name, 4-bytes of
flags, a pointer to a list of parameters, and a code pointer. The parameter list
starts with a 1-byte return type, followed by a 1-byte count of parameters, and
ends with a list of 1-byte parameter types. The parameter names are not stored.
Example 3.8 shows the declarations for the automated method table. Example 3.8.
The Layout of the Automated Method Table
...
Initialization and Finalization
        ...
        Delphi must decrement the reference counts for strings, interfaces,
dynamic arrays, and free Variants and wide strings. To keep track of this
information, Delphi uses initialization records as part of a class's RTTI. In
fact, every record and array that requires finalization has an associated
initialization record, but the compiler hides these records. The only ones you
have access to are those associated with an object's fields. A VMT points to an
initialization table. The table contains a list of initialization records.
Because arrays and records can be nested, each initialization record contains a
pointer to another initialization table, which can contain initialization
records, and so on. An initialization table uses a TTypeKind field to keep track
of whether it is initializing a string, a record, an array, etc. An
initialization table begins with the type kind (1 byte), followed by the type
name as a short string, a 4-byte size of the data being initialized, a 4-byte
count for initialization records, and then an array of zero or more
        initialization records. An initialization record is just a pointer to a
        nested initialization table, followed by a 4-byte offset for the field
that must be initialized. Example 3.7 shows the logical layout of the
initialization table and record, but the declarations depict the logical layout
without being true Pascal code. Example 3.7. The Layout of the Initialization
Table and Record
        ...
        type
          ...
          {$MinEnumSize 1} // Ensure that TypeKind takes up 1 byte.
                ...
                // has only one element; if the type kind is tkRecord, Count is
the
...
MsgInfo WindowsMsgTab[] = { { 1, "WMCreate", "WM_CREATE" },
...
MsgInfo VCLControlsMsgTab[] = { { 0xB000, "CMActivate", "CM_ACTIVATE" },
...
// ----Highlighting-----------------------------------------------------------
          if (InitHighlight())
          {
              ...
              DelphiLbId		  =
CreateHighlight(lbSourceCode->Handle, ID_DELPHI); DelphiThemesCount   =
GetThemesCount(ID_DELPHI); for (n = 0; n < DelphiThemesCount; n++)
              {
...
void __fastcall TFMain_11011981::ScanImports()
{
        ...
        //Ïîïðîáóåì ñêàíèðîâàòü èíòåðâàë àäðåñîâ áåçûìÿííûõ ìîäóëåé ïî èìåíàì
èìïîðòèðóåìûõ ôóíêöèé for (int m = 0; m < UnitsNum; m++)
        {
                ...
                if (!recU->names->Count)
                {
                        ...
                        for (int n = fromPos; n < toPos; n++)
                        {
                                ...
                                if (recN && IsFlagSet(cfImport, n))
                                {
                                        ...
                                        for (int k = 0;;k++)
                                        {
                                                if (KnowledgeBase.Mods[k] ==
0xFFFF) break; cnt[KnowledgeBase.Mods[k]]++; totcnt++;
                                        }
                                }
                        }
                        ...
                        {
                                int num = 0; WORD id;
                                for (int k = 0; k < KnowledgeBase.ModuleCount;
k++)
                                {
                                        if (cnt[k] == totcnt)
                                        {
                                ...
                                //Åñëè âñå èìïîðòû íàøëèñü òîëüêî â îäíîì þíèòå,
çíà÷èò ýòî îí è åñòü if (num == 1)
                                {
                                ...
                                //Åñëè â íåñêîëüêèõ, ïîïðîáóåì ïîèñêàòü
ïðîöåäóðû ïî cfProcStart (åñëè òàêîâûå èìåþòñÿ) else
                                {
                                        for (int k = 0; k <
KnowledgeBase.ModuleCount; k++)
                                        {
                                                if (cnt[k] == totcnt)
                                                {
                                                        id = k;
                                                        int FirstProcIdx,
LastProcIdx; if (!KnowledgeBase.GetProcIdxs(id, &FirstProcIdx, &LastProcIdx))
continue;
...
< toPos; m++)
                                                        {
                                                                if
(IsFlagSet(cfProcStart, m) || !Flags[m])
                                                                {
                                                                        for (int
Idx = FirstProcIdx; Idx <= LastProcIdx; Idx++)
                                                                        {
                                                                                Idx = KnowledgeBase.ProcOffsets[Idx].NamId;
                                                                                if (!KnowledgeBase.IsUsedProc(Idx))
                                                                                {
...
void __fastcall TFMain_11011981::AnalyzeMethodTable(int Pass, DWORD Adr, const bool* Terminated)
{
	...
	if(DelphiVersion >= 2010) {
		...
		for(int n = 0; n < exCount && !*Terminated; n++) {
			...
			recN = GetInfoRec(procAdr);
			// IMHO it means that methods are pure virtual calls and must be readed in
			// child classes
...
void __fastcall TFMain_11011981::miEditClassClick(TObject* Sender)
{
	if(ActiveControl == tvClassesShort) {
		...
		if(node) {
			int FieldOfs = -1;
			if(!node->Text.Pos("#"))
				swscanf(String(node->Text).c_str(), L"%lX", &FieldOfs);
			while(node) {
	...
	if(version == DELHPI_VERSION_AUTO) // Autodetect
	{
		...
		if(DelphiVersion == 1) {
			...
			ShowMessage("File " + FileName +
						" is probably Delphi 4, 5, 6, 7, 2005, 2006 or 2007 file, "
...
bool __fastcall TFMain_11011981::ImportsValid(DWORD ImpRVA, DWORD ImpSize)
{
	if(ImpRVA || ImpSize) {
		...
		while(1) {
			...
			if(!ImportDescriptor.OriginalFirstThunk && !ImportDescriptor.TimeDateStamp && !ImportDescriptor.ForwarderChain && !ImportDescriptor.Name &&
...
int __fastcall TFMain_11011981::LoadImage(FILE* f, int version, bool loadExp, bool loadImp)
{
	...
	if(loadImp && (ImpRVA || ImpSize)) {
		if(!ImportsValid(ImpRVA, ImpSize)) {
			...
			DWORD
			ThunkValue; // Value of next thunk (from OriginalFirstTunk or FirstThunk)
			...
			while(1) {
				...
				// HINSTANCE hLib = LoadLibraryEx(moduleName.c_str(), 0,
				...
				while(1) {
					// Names or ordinals get from LookupTable (this table can be inside
					// OriginalFirstThunk or FirstThunk)
...
void __fastcall TFMain_11011981::OpenProject(String FileName)
{
	...
	if(fIn) {
		...
		for(n = 0; n < num; n++) {
			PSegmentInfo segInfo = new SegmentInfo;
			fread(&segInfo->Start, sizeof(segInfo->Start), 1,
				  fIn); // inStream->Read(&segInfo->Start, sizeof(segInfo->Start));
			fread(&segInfo->Size, sizeof(segInfo->Size), 1,
				  fIn); // inStream->Read(&segInfo->Size, sizeof(segInfo->Size));
			fread(&segInfo->Flags, sizeof(segInfo->Flags), 1,
				  fIn);                       // inStream->Read(&segInfo->Flags, sizeof(segInfo->Flags));
		...
		if(Items)
			fread(pImage, Items, 1, fIn); // inStream->Read(pImage, Items);
		...
		while(Items >= MAX_ITEMS) {
			fread(pFlags, sizeof(DWORD) * MAX_ITEMS, 1,
				  fIn); // inStream->Read(pFlags, sizeof(DWORD)*MAX_ITEMS);
			pFlags += MAX_ITEMS;
			Items -= MAX_ITEMS;
		}
		if(Items)
			fread(pFlags, sizeof(DWORD) * Items, 1, fIn); // inStream->Read(pFlags, sizeof(DWORD)*Items);
		...
		RTTISortField = 0;
		if(num)
			fread(&RTTISortField, sizeof(RTTISortField), 1, fIn); // inStream->Read(&RTTISortField, sizeof(RTTISortField));
		// UpdateRTTIs
		...
		for(n = 0; n < num; n++) {
			...
			// MemStream
			fread(&size, sizeof(size), 1,
				  fIn); // inStream->Read(&size, sizeof(size));
			...
			fread(&evnum, sizeof(evnum), 1,
				  fIn); // inStream->Read(&evnum, sizeof(evnum));
			for(m = 0; m < evnum; m++) {
			...
			// Components
			fread(&cnum, sizeof(cnum), 1,
				  fIn); // inStream->Read(&cnum, sizeof(cnum));
			if(cnum) {
				...
				for(m = 0; m < cnum; m++) {
					...
					// HasGlyph
					fread(&cInfo->HasGlyph, sizeof(cInfo->HasGlyph), 1, fIn); // inStream->Read(&cInfo->HasGlyph,
					// sizeof(cInfo->HasGlyph));
					// Name
					fread(&len, sizeof(len), 1,
						  fIn);              // inStream->Read(&len, sizeof(len));
					fread(buf, len, 1, fIn); // inStream->Read(buf, len);
...
void __fastcall TFMain_11011981::SaveProject(String FileName)
{
	...
	try {
		...
		if(outF) {
			...
			for(n = 0; n < num; n++) {
				PSegmentInfo segInfo = (PSegmentInfo)SegmentList->Items[n];
				fwrite(&segInfo->Start, sizeof(segInfo->Start), 1, outF); // outStream->Write(&segInfo->Start,
																		  // sizeof(segInfo->Start));
				fwrite(&segInfo->Size, sizeof(segInfo->Size), 1, outF);   // outStream->Write(&segInfo->Size, sizeof(segInfo->Size));
				fwrite(&segInfo->Flags, sizeof(segInfo->Flags), 1, outF); // outStream->Write(&segInfo->Flags,
																		  // sizeof(segInfo->Flags));
			...
			if(Items)
				fwrite(pImage, Items, 1, outF); // outStream->Write(pImage, Items);
			...
			if(Items)
				fwrite(pFlags, sizeof(DWORD), Items, outF); // outStream->Write(pFlags, sizeof(DWORD)*Items);
			...
			for(n = 0; n < num; n++) {
				...
				fwrite(&recU->trivialIni, sizeof(recU->trivialIni), 1, outF); // outStream->Write(&recU->trivialIni,
																			  // sizeof(recU->trivialIni));
				fwrite(&recU->trivialFin, sizeof(recU->trivialFin), 1, outF); // outStream->Write(&recU->trivialFin,
																			  // sizeof(recU->trivialFin));
				...
				fwrite(&recU->iniOrder, sizeof(recU->iniOrder), 1, outF);     // outStream->Write(&recU->iniOrder,
																			  // sizeof(recU->iniOrder));
			...
			if(num)
				fwrite(&RTTISortField, sizeof(RTTISortField), 1, outF); // outStream->Write(&RTTISortField, sizeof(RTTISortField));
			...
			for(n = 0; n < num; n++) {
				...
				for(m = 0; m < cnum; m++) {
					...
					// Inherited
					fwrite(&cInfo->Inherit, sizeof(cInfo->Inherit), 1, outF); // outStream->Write(&cInfo->Inherit,
																			  // sizeof(cInfo->Inherit));
					// HasGlyph
					fwrite(&cInfo->HasGlyph, sizeof(cInfo->HasGlyph), 1, outF); // outStream->Write(&cInfo->HasGlyph,
																				// sizeof(cInfo->HasGlyph));
					// Name
			...
			fwrite(&num, sizeof(num), 1,
				   outF); // outStream->Write(&num, sizeof(num));
			for(n = 0; n < num; n++) {
				FProgressBar->pb->StepIt();
				Application->ProcessMessages();
				...
				if(len > MaxBufLen)
					MaxBufLen = len;
				fwrite(&len, sizeof(len), 1, outF);                         // outStream->Write(&len, sizeof(len));
				fwrite(ResInfo->Aliases->Strings[n].c_str(), len, 1, outF); // outStream->Write(ResInfo->Aliases->Strings[n].c_str(),
																			// len);
			}

			// CodeHistory
			fwrite(&CodeHistorySize, sizeof(CodeHistorySize), 1, outF); // outStream->Write(&CodeHistorySize,
																		// sizeof(CodeHistorySize));
			...
			for(n = 0; n < CodeHistorySize; n++) {
				FProgressBar->pb->StepIt();
				Application->ProcessMessages();
				fwrite(&CodeHistory[n], sizeof(PROCHISTORYREC), 1, outF); // outStream->Write(&CodeHistory[n],
																		  // sizeof(PROCHISTORYREC));
			}
			...
			// Important variables
			fwrite(&HInstanceVarAdr, sizeof(HInstanceVarAdr), 1, outF); // outStream->Write(&HInstanceVarAdr,
																		// sizeof(HInstanceVarAdr));
...
void __fastcall TFMain_11011981::miMapGeneratorClick(TObject* Sender)
{
	...
	if(_posMap)
		SourceFileMap = SourceFileMap.SubString(_posMap + 1, SourceFileMap.Length());
		...
		if(IsFlagSet(cfProcStart, n) && !IsFlagSet(cfEmbedded, n)) {
			...
			if(recN) {
				if(adr != EP) {
					...
					// {
					// fprintf(fMap, "%lX %s\n", adr, recN->MakePrototype(adr, true, true,
					// false, true, false).c_str());
					// }
				} else {
...
void __fastcall TFMain_11011981::OutputCode(FILE* outF, DWORD fromAdr, String prototype, bool onlyComments)
{
	...
	while(row < outRows) {
       ...
       */
			...
			// bool neg = false;
			// Adr1 = *((DWORD*)(Code + Pos - 4));
			// if (IsValidCodeAdr(Adr1) && IsFlagSet(cfLoc, Adr2Pos(Adr1))) neg =
			// true;
...
void __fastcall TFMain_11011981::wm_dropFiles(TWMDropFiles& msg)
{
	...
	try {
		for(int i = 0; i < fc->FileCount; ++i) {
			...
			// Processed the first - and go out - we cannot process more than one file
			// yet
			break;
		}
...
void __fastcall TFMain_11011981::miCtdPasswordClick(TObject* Sender)
{
	...
	/*
      ...
      {
              instrLen = Disasm.Disassemble(Code + curPos, (__int64)curAdr,
     &DisInfo); op = Disasm.GetOp(DisInfo.Mnem);
              //mov [ebp-Ofs], B
              if (op == OP_MOV && DisInfo.Op1Type == otMEM && DisInfo.Op2Type ==
     otIMM && DisInfo.BaseReg == 21 && (int)DisInfo.Offset < 0)
              {
                      ofs = DisInfo.Offset; if (128 + ofs < beg) beg = 128 +
     ofs; pwd[128 + ofs] = DisInfo.Immediate; n++;
              }
              curPos += instrLen; curAdr += instrLen;
      }
...
int __fastcall GetRecordField(String ARecType, int AOfs, String& name, String& type)
{
				...
				if(_tInfo.FieldsNum) {
					...
					for(n = 0; n < _tInfo.FieldsNum; n++) {
						...
						if(AOfs >= Ofs1 && AOfs < Ofs2) {
							...
							_len = *((WORD*)p);
							p += 2;
							name = AnsiString(p, _len);
							p += _len + 1;
							_len = *((WORD*)p);
							p += 2;
							type = AnsiString(p, _len);
							return Ofs1;
						}
					}
				} else if(_tInfo.Decl != "") {
	...
	if(_recT && _recT->kind == ikRecord) {
		...
		_len = Code[_pos];
		_pos++;
		_name = AnsiString((char*)(Code + _pos), _len);
		_pos += _len; // Name
		_pos += 4;    // Size
...
void __fastcall TFMain_11011981::SetupAllFonts(TFont* font)
{
	TListBox* formListBoxes[] = {
		lbUnits, lbRTTIs, lbForms, lbAliases, lbCode, lbStrings, lbNames, lbNXrefs, lbSXrefs, lbCXrefs, lbSourceCode, lbUnitItems, 0
	};
...
void __fastcall TFMain_11011981::miHiewGeneratorClick(TObject* Sender)
{
	...
	if(SourceFile != "")
		nametName = ChangeFileExt(SourceFile, ".namet");
	if(IDPFile != "")
		nametName = ChangeFileExt(IDPFile, ".namet");
	...
	if(_posNamet)
		SourceFileNamet = SourceFileNamet.SubString(_posNamet + 1, SourceFileNamet.Length());
	for(int n = 0; n < CodeSize; n++) {
		if(IsFlagSet(cfProcStart, n) && !IsFlagSet(cfEmbedded, n)) {
			int adr = Pos2Adr(n);
			PInfoRec recN = GetInfoRec(adr);
...
CPPvsDELPHIdecl ForwardDeclarations[] = { { "bool", "Boolean" },
										  { "char", "AnsiChar" },
...
{
	for(int n = 0;; n++) {
		if(!ForwardDeclarations[n].CppType)
			break;
...
void __fastcall TFMain_11011981::OutputForwardDeclarationsHeader(FILE* hF)
{
	...
	fprintf(hF,
			"//It may require manually editing, because of arrays and order "
			"mismatching.\n");
	...
	fprintf(hF,
			"//You can leave only one variant or try convert to union "
			"(question is why?).\n\n");
		...
		for(adr = recU->fromAdr; adr < recU->toAdr; adr++) {
			...
			switch(kind) {
				...
				case ikRecord:
					...
					if(SameText(RTTIName, "TVarData") || SameText(RTTIName, "TVarRec")) {
						break;
					}
					// shadow name
					if(RTTIName.Pos(":") > 0)
