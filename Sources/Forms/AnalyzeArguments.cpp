						...
						if(b & cfInstruction) {
							...
							if(b & cfPop) {
								...
								if(b & cfSetA) {
									...
									if(!kb && !(recN1->procInfo->flags & (PF_EVENT | PF_DYNAMIC)) && recN1->kind != ikConstructor &&
		...
		while(1) {
			...
			if(op == OP_CDQ) {
				if(lastCallAdr) {
					...
					if(recN1 && recN1->procInfo && !(recN1->procInfo->flags & (PF_KBPROTO | PF_EVENT | PF_DYNAMIC)) && recN1->kind != ikConstructor &&
					   recN1->kind != ikDestructor) {
						recN1->procInfo->flags |= PF_OUTEAX;
						recN1->kind = ikFunc;
					}
