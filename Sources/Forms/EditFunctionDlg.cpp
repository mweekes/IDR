...
void __fastcall TFEditFunctionDlg_11011981::bEditClick(TObject* Sender)
{
	if(pc->ActivePage == tsType) {
		...
		if(locInfo) {
			...
			edtVarSize->Text = AnsiString(locInfo->Size);
			edtVarName->Text = AnsiString(locInfo->Name);
			edtVarType->Text = AnsiString(locInfo->TypeDef);
		}
...
void __fastcall TFEditFunctionDlg_11011981::bApplyVarClick(TObject* Sender)
{
	...
	/*
	 ...
	 AnsiString str = AnsiString(StringBuf);
	 if (str.Pos(ftype + "=") == 1)
	 {
	 while (1)
	 {
	 if (!fgets(StringBuf, 1024, recFile)) break;
	 str = AnsiString(StringBuf);
	 if (str.Pos("end;")) break;
	 int pos2 = str.Pos("//");
	 if (pos2)
	 {
	 AnsiString ofs = str.SubString(pos2 + 2, str.Length());
	 int pos1 = str.Pos(":");
	 if (pos1)
	 {
	 ...
	 //Name
	 int len = *((WORD*)p); p += 2;
	 name = AnsiString((char*)p, len); p += len + 1;
	 //Type
	 len = *((WORD*)p); p += 2;
...
void __fastcall TFEditFunctionDlg_11011981::FillType()
{
	...
	line = recN->MakeMultilinePrototype(Adr, &argsBytes, (cbMethod->Checked) ? cbVmtCandidates->Text : String(L""));
	...
	lRetBytes->Caption = AnsiString(recN->procInfo->retBytes);
	lArgsBytes->Caption = AnsiString(argsBytes);
}
