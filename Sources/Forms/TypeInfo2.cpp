...
String __fastcall TFTypeInfo_11011981::GetRTTI(DWORD adr)
{
	...
	switch(RTTIKind) {
		...
		case ikEnumeration:
			...
			break;
		case ikFloat:
			if(SameText(RTTIName, "Single") || SameText(RTTIName, "Double") || SameText(RTTIName, "Extended") || SameText(RTTIName, "Comp") ||
			...
			*o_pSize = *((DWORD*)(Code + pos));
			...
			for(i = 0; i < fieldsList->Count; i++) {
				...
				if(FieldInfo->Type != "?" && FieldInfo->Type != "") {
					...
					if(typeKind == ikEnumeration)
						size = 1;
					//if (typeKind == ikSet) size = 1;
					if(typeKind == ikMethod)
						size = 8;
					if(typeKind == ikInterface)
						size = 4;
