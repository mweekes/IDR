								...
								if(DisInfo.Call) {
									...
									if(IsValidCodeAdr(Adr)) {
										...
										if(recN1 && recN1->procInfo /*recN1->kind == ikFunc*/) {
											...
											if(!(recN1->procInfo->flags & (PF_EVENT | PF_DYNAMIC)) && recN1->kind != ikConstructor &&
													...
													if(IsValidImageAdr(Val)) {
														...
														if(_ap >= 0) {
															...
															if(recN1) {
																...
																varName = recN1->GetName();
																if(varName != "" &&
																   (recN1->kind == ikLString || recN1->kind == ikWString || recN1->kind == ikUString))
																	varName = "\"" + varName + "\"";
																if(varName != "")
																	recN->SetName("^" + varName);
																if(recN->type != "")
			...
			if(IsValidImageAdr(itemAdr)) {
				...
				if(itemPos >= 0) {
					...
					if(!recN1 || recN1->kind != ikVMT) {
						...
						if(SameText(typeDef, "PShortString") || SameText(typeDef, "ShortString")) {
						...
						} else if(SameText(typeDef, "AnsiString") || SameText(typeDef, "String") || SameText(typeDef, "UString") ||
							...
							if(!recN1)
								recN1 = new InfoRec(itemPos, ikData);
							if(!recN1->HasName() && recN1->kind != ikProc && recN1->kind != ikFunc && recN1->kind != ikConstructor &&
							   recN1->kind != ikDestructor && recN1->kind != ikRefine) {
								if(typeDef != "")
									recN1->type = typeDef;
							}
						}
