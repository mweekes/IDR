...
void __fastcall TFMain_11011981::AnalyzeProc1(DWORD fromAdr, char xrefType, DWORD xrefAdr, int xrefOfs, bool maybeEmb)
{
	...
	while(1) {
		...
		////fstp [esp]
		// if (!memcmp(DisInfo.Mnem, "fst", 3) && DisInfo.BaseReg == 20)
		// SetFlag(cfFush, curPos);
		...
		// Is instruction (not call and jmp), that contaibs operand [reg+xxx], where
		// xxx is negative value
		if(maybeEmb && !DisInfo.Call && !DisInfo.Branch && (int)DisInfo.Offset < 0 &&
		   (DisInfo.IndxReg == -1 && (DisInfo.BaseReg >= 16 && DisInfo.BaseReg <= 23 && DisInfo.BaseReg != 20 && DisInfo.BaseReg != 21))) {
			// May be add condition that all Xrefs must points to one
			// subroutine!!!!!!!!!!!!!
						...
						if(Code[Pos] == 0xE9) // jmp Handle...
						{
							...
							if(recN1 && recN1->HasName()) {
								...
								if(recN1->SameName("@HandleFinally")) {
									...
									if(Code[Pos] == 0x68) // push offset @3       //Flags[Pos] &
														  // cfInstruction must be != 0
									{
										hfStartPos = Pos - 8;
		...
		if(DisInfo.Call) {
			...
			if(IsValidCodeAdr(Adr) && Adr2Pos(Adr) >= 0) {
				...
				// If after call exists instruction pop ecx, it may be embedded
				// procedure
				...
				if(recN1 && recN1->procInfo) {
					...
					if(recN1->HasName()) {
						...
						int begPos, endPos;
						// If called procedure is @ClassCreate, then current procedure is
						// constructor
						if(recN1->SameName("@ClassCreate")) {
							recN->kind = ikConstructor;
							// Code from instruction test... until this call is not sufficient
							// (mark skipped)
						...
						// If called procedure is @ClassDestroy, then current procedure is
						// destructor
		...
		if(b1 == 0xEB || // short relative abs jmp or cond jmp
		   (b1 >= 0x70 && b1 <= 0x7F) || (b1 == 0xF && b2 >= 0x80 && b2 <= 0x8F)) {
			...
			if(IsValidCodeAdr(Adr)) {
				Pos = Adr2Pos(Adr);
				if(!IsFlagSet(cfEmbedded, Pos)) // Possible branch to start of Embedded
												// proc (for ex. in proc TextToFloat))
				{
					SetFlag(cfLoc, Pos);
		...
		if(b1 == 0xE9) // relative abs jmp or cond jmp
		{
			...
			if(IsValidCodeAdr(Adr)) {
				...
				recN1 = GetInfoRec(Adr);
				if(recN1 && recN1->HasName()) {
					if(recN1->SameName("@HandleFinally") || recN1->SameName("@HandleAnyException") || recN1->SameName("@HandleOnException") ||
