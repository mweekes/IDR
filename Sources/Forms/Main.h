...
#define cfEmbedded \
    0x00000080 //Calls in range of one proc (for ex. calls in FormatBuf)
...
#define cfFinallyExit \
    0x10000000 //Exit section (from try...finally construction)
#define cfVTable \
    0x20000000 //Flags for Interface entries (to mark start end finish of VTables)
...
typedef struct
{
    ...
    char
        source; // 0 - not defined; 'L' - local var; 'A' - argument; 'M' - memory; 'I' - immediate
    DWORD value;
    String type;
} RINFO, *PRINFO;
// ---------------------------------------------------------------------------
...
class TFMain_11011981 : public TForm
{
  __published: // IDE-managed Components
    ...
    void __fastcall lbUnitsDrawItem(
        TWinControl* Control, int Index, TRect &Rect, TOwnerDrawState State);
    void __fastcall FormKeyDown(TObject* Sender, WORD &Key, TShiftState Shift);
    void __fastcall lbCodeKeyDown(
    ...
    void __fastcall lbUnitItemsDrawItem(
        TWinControl* Control, int Index, TRect &Rect, TOwnerDrawState State);
    void __fastcall lbUnitsMouseMove(
        TObject* Sender, TShiftState Shift, int X, int Y);
    void __fastcall lbRTTIsMouseMove(
        TObject* Sender, TShiftState Shift, int X, int Y);
    void __fastcall lbFormsMouseMove(
        TObject* Sender, TShiftState Shift, int X, int Y);
    void __fastcall lbCodeMouseMove(
        TObject* Sender, TShiftState Shift, int X, int Y);
    void __fastcall tvClassesFullMouseMove(
        TObject* Sender, TShiftState Shift, int X, int Y);
    void __fastcall lbStringsMouseMove(
        TObject* Sender, TShiftState Shift, int X, int Y);
    void __fastcall lbUnitItemsMouseMove(
        TObject* Sender, TShiftState Shift, int X, int Y);
    void __fastcall lbXrefsMouseMove(
        TObject* Sender, TShiftState Shift, int X, int Y);
    void __fastcall miSortUnitsByNamClick(TObject* Sender);
    void __fastcall rgViewerModeClick(TObject* Sender);
    void __fastcall tvClassesShortMouseMove(
        TObject* Sender, TShiftState Shift, int X, int Y);
    ...
    void __fastcall lbXrefsKeyDown(
        TObject* Sender, WORD &Key, TShiftState Shift);
    void __fastcall lbUnitsKeyDown(
        TObject* Sender, WORD &Key, TShiftState Shift);
    void __fastcall lbRTTIsKeyDown(
        TObject* Sender, WORD &Key, TShiftState Shift);
    void __fastcall lbFormsKeyDown(
        TObject* Sender, WORD &Key, TShiftState Shift);
    void __fastcall tvClassesShortKeyDown(
        TObject* Sender, WORD &Key, TShiftState Shift);
    void __fastcall lbUnitItemsKeyDown(
        TObject* Sender, WORD &Key, TShiftState Shift);
    ...
    void __fastcall lbSourceCodeMouseMove(
        TObject* Sender, TShiftState Shift, int X, int Y);
    void __fastcall cbMultipleSelectionClick(TObject* Sender);
    void __fastcall lbSourceCodeDrawItem(
        TWinControl* Control, int Index, TRect &Rect, TOwnerDrawState State);
  ...
  private: // User declarations
	...
	void __fastcall ReadNode(TStream* stream, TTreeNode* node, char* buf); //this might be old
    ...
    void __fastcall WriteNode(FILE* f,
        TTreeNode*
            node); //void __fastcall WriteNode(TStream* stream, TTreeNode* node);
    ...
    void __fastcall OutputLine(
        FILE* outF, BYTE flags, DWORD adr, String content);
    void __fastcall OutputCode(
        FILE* outF, DWORD fromAdr, String prototype, bool onlyComments);
  ...
  public: // User declarations
    ...
    void __fastcall DoOpenDelphiFile(
        int version, String FileName, bool loadExp, bool loadImp);
    ...
    int __fastcall GetField(
        String TypeName, int Offset, String &name, String &type);
    PFIELDINFO __fastcall GetField(
        String TypeName, int Offset, bool* vmt, DWORD* vmtAdr, String prefix);
    PFIELDINFO __fastcall AddField(DWORD ProcAdr, int ProcOfs, String TypeName,
        BYTE Scope, int Offset, int Case, String Name, String Type);
    ...
    void __fastcall StrapProc(
        int pos, int ProcId, MProcInfo* ProcInfo, bool useFixups, int procSize);
    ...
    void __fastcall ShowCode(
        DWORD fromAdr, int SelectedIdx, int XrefIdx, int topIdx);
    void __fastcall AnalyzeProc(int pass, DWORD procAdr);
    void __fastcall AnalyzeMethodTable(
        int pass, DWORD adr, const bool* Terminated);
    void __fastcall AnalyzeDynamicTable(
        int pass, DWORD adr, const bool* Terminated);
    void __fastcall AnalyzeVirtualTable(
        int pass, DWORD adr, const bool* Terminated);
    DWORD __fastcall AnalyzeProcInitial(DWORD fromAdr);
    void __fastcall AnalyzeProc1(DWORD fromAdr, char xrefType, DWORD xrefAdr,
        int xrefOfs, bool maybeEmb);
    void __fastcall AnalyzeProc2(
        DWORD fromAdr, bool addArg, bool AnalyzeRetType);
    bool __fastcall AnalyzeProc2(
        DWORD fromAdr, bool addArg, bool AnalyzeRetType, TList* sctx);
    String __fastcall AnalyzeTypes(
        DWORD parentAdr, int callPos, DWORD procAdr, PRINFO registers);
    ...
    void __fastcall FillClassViewerOne(
        int n, TStringList* tmpList, const bool* terminated);
    ...
    BEGIN_MESSAGE_MAP
        ...
        VCL_MESSAGE_HANDLER(
            WM_UPDANALYSISSTATUS, TMessage, wm_updAnalysisStatus);
