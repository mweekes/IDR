...
// ---------------------------------------------------------------------------
bool __stdcall EnumResNameProcedure(int hModule, WideChar* Type, UnicodeString Name, long Param)
{
	bool res;
	TFilerFlags flags;
		...
		if(Type == RT_RCDATA) {
			...
			if(ResInfo->hFormPlugin) {
				...
				if(res) {
					...
					BYTE len = *(mp + 4);
					res = (len == Name.Length() && SameText(Name, String((char*)mp + 5, len)));
				}
			}
			if(res) {
...
{
	citadel = false;
	hFormPlugin = LoadLibrary(String((FMain_11011981->AppDir + "Plugins\\" + FormPluginName)).c_str());
	HINSTANCE hInst = LoadLibraryEx(String(FileName).c_str(), 0, LOAD_LIBRARY_AS_DATAFILE);
	if(hInst) {
...
// IdrDfmReader
// ---------------------------------------------------------------------------
__fastcall IdrDfmReader::IdrDfmReader(TStream* Stream, int BufSize)
...
// TComponentEvents
// ---------------------------------------------------------------------------
__fastcall TComponentEvents::TComponentEvents(TCollection* Owner)
...
// IdrDfmForm
// ---------------------------------------------------------------------------
__fastcall IdrDfmForm::IdrDfmForm(TComponent* Owner)
  : TForm(Owner)
{
	compsEventsList = 0;
	current = 0;
...
// ---------------------------------------------------------------------------
__fastcall IdrDfmForm::IdrDfmForm(TComponent* Owner, int Dummy)
  : TForm(Owner, Dummy)
{
	compsEventsList = 0;
	current = 0;
...
// IdrDfmLoader
// ---------------------------------------------------------------------------
__fastcall IdrDfmLoader::IdrDfmLoader(TComponent* Owner)
  : TComponent(Owner)
{
	dfmForm = 0;
	Counter = 0;
...
void __fastcall IdrDfmLoader::ReaderError(TReader* Reader, String Message, bool& Handled)
{
	...
	if(AnsiEndsStr("Invalid property value", Message) || AnsiEndsStr("Unable to insert a line", Message) ||
...
__fastcall IdrDfmDefaultControl::IdrDfmDefaultControl(TComponent* Owner)
  : TPanel(Owner)
{
	Color = clNone;
	imgIcon = 0;
...
void IdrDfmDefaultControl::CreateImageIconForClass(const String& imgFile)
{
	...
	if(!IconsDllLoaded) {
		hResLib = LoadLibrary(L"Icons.dll");
...
// IdrImageControl
// ---------------------------------------------------------------------------
__fastcall IdrImageControl::IdrImageControl(TComponent* Owner)
  : TImage(Owner)
{}
