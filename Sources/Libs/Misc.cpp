...
{
	SetTextBuf(Value.c_bstr());
}
//---------------------------------------------------------------------------
WideString __fastcall UnicodeEncode(AnsiString Str, int CodePage)
{
	int Len;
	WideString Result;
	...
	Len = MultiByteToWideChar(CodePage, 0, PAnsiChar(Str.c_str()), -1, PWideChar(Result), Len);
	Result.SetLength(Len - 1); // end is #0
	return Result;
};
//---------------------------------------------------------------------------
String __fastcall UnicodeDecode(UnicodeString Str, int CodePage)
{
	int Len;
	AnsiString Result;
	...
	Result.SetLength(Len);
	Len = WideCharToMultiByte(CodePage, 0, PWideChar(Str.c_str()), -1, PAnsiChar(Result.c_str()), Len, 0, 0);
...
{
	if(Adr >> 8 == 0x408D)
		return false; // Add by ZGL assigned global byte or boolean variable
...
// ---------------------------------------------------------------------------
String __fastcall ExtractProcName(const AnsiString& AName)
{
	if(AName == "")
		return "";
	int pos = AName.Pos(".");
...
{
	// !!!
	if(pos < 0 || pos >= (int)TotalSize) {
		dummy = 1;
		return false;
	}
	assert(pos >= 0 && pos < (int)TotalSize);
...
{
	// !!!
	if(pos < 0 || pos + num >= (int)TotalSize) {
		dummy = 1;
		return;
	}
	assert(pos >= 0 && pos + num < (int)TotalSize);
...
String __fastcall GetRecordFields(int AOfs, String ARecType)
{
	...
	if(ARecType == "" || GetField(ARecType, AOfs, _name, _typeName) < 0)
		return "";
...
String TypeKinds[TypeKindsNum] = { "Unknown", "Integer",  "Char",       "Enumeration", "Float",   "ShortString", "Set",    "Class",
								   "Method",  "WChar",    "AnsiString", "WideString",  "Variant", "Array",       "Record", "Interface",
...
BYTE __fastcall GetTypeKind(String AName, int* size)
{
	...
	*size = 4;
	if(AName != "") {
		...
		if(SameText(name, "ShortInt") || SameText(name, "SmallInt") || SameText(name, "Integer") || SameText(name, "LongInt") || SameText(name, "LongWord") ||
		...
		if(SameText(name, "Real48") || SameText(name, "Real") || SameText(name, "Double") || SameText(name, "TDate") || SameText(name, "TTime") ||
		   SameText(name, "TDateTime") || SameText(name, "Comp") || SameText(name, "Currency")) {
			*size = 8;
			return ikFloat;
		}
...
int __fastcall GetPackedTypeSize(String AName)
{
	...
	if(SameText(AName, "Dword") || SameText(AName, "Integer") || SameText(AName, "LongInt") || SameText(AName, "LongWord") || SameText(AName, "Cardinal") ||
...
String __fastcall GetEnumerationString(String TypeName, Variant Val)
{
	...
	if(IsValidImageAdr(adr)) {
		...
		for(n = (int)minValueB; n <= (int)maxValueB; n++) {
			len = Code[pos];
			pos++;
			if(n >= (int)minValue && n <= (int)maxValue && n == _val) {
				return String((char*)(Code + pos), len);
			}
			pos += len;
		}
...
bool __fastcall GetArrayIndexes(AnsiString arrType, int ADim, int* LowIdx, int* HighIdx)
{
	char c, *p, *b;
	int _dim, _val, _pos;
	AnsiString _item, _item1, _item2;
...
String __fastcall GetModuleVersion(const String& module)
{
	...
	AnsiString strVersion = ""; // empty means not found, etc - some error
	...
	if(hLib) {
		...
		if(pHeader && pHeader->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT].VirtualAddress &&
		   pHeader->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT].Size) {
			...
			for(int i = 0; i < (int)(pExport->NumberOfFunctions); i++) {
				int index = -1;
				for(int j = 0; j < (int)(pExport->NumberOfNames); j++) {
...
bool __fastcall IsBplByExport(const char* bpl)
{
	...
	HMODULE hLib = LoadLibraryEx(String(bpl).c_str(), 0, LOAD_LIBRARY_AS_DATAFILE);
	imageBase = (DWORD)(DWORD_PTR)hLib;
...
int __fastcall IsInlineLengthTest(DWORD fromAdr)
{
	...
	if(_dd == 'tset' && _disInfo.OpType[0] == otREG && _disInfo.OpType[1] == otREG && _disInfo.OpRegIdx[0] == _disInfo.OpRegIdx[1]) {
		...
		if(_dd == 'zj' || _dd == 'ej') {
			...
			_dd = *((DWORD*)_disInfo.Mnem);
			// mov reg, [reg-4]
			if(_dd == 'vom' && _disInfo.OpType[0] == otREG && _disInfo.OpType[1] == otMEM && _disInfo.BaseReg == _regIdx && _disInfo.IndxReg == -1 &&
			   _disInfo.Offset == -4) {
		...
		if(_dd == 'zj' || _dd == 'ej') {
			...
			if(_op == OP_MOV && _disInfo.OpType[0] == otREG && _disInfo.OpType[1] == otMEM && _disInfo.BaseReg == _baseReg && _disInfo.Offset == _offset) {
				...
				if(_op == OP_SUB && _disInfo.OpType[0] == otREG && _disInfo.OpType[1] == otIMM && _disInfo.Immediate == 4) {
					...
					_op = Disasm.GetOp(_disInfo.Mnem);
					// mov reg, [reg]
					if(_op == OP_MOV && _disInfo.OpType[0] == otREG && _disInfo.OpType[1] == otMEM && _disInfo.BaseReg == _regIdx && _disInfo.IndxReg == -1 &&
					   _disInfo.Offset == 0) {
						_curPos += _instrLen;
						_curAdr += _instrLen;
						_instrLen = Disasm.Disassemble(Code + _curPos, (__int64)_curAdr, &_disInfo, 0);
						_op = Disasm.GetOp(_disInfo.Mnem);
						// mov [lvar], reg
						if(_op == OP_MOV && _disInfo.OpType[0] == otMEM && _disInfo.OpType[1] == otREG && _disInfo.BaseReg == _baseReg &&
						   _disInfo.Offset == (unsigned int)_offset) {
							if(_adr == _curAdr + _instrLen)
								return (_curAdr + _instrLen) - fromAdr;
						}
					}
	...
	if(_op == OP_AND && _disInfo.OpType[0] == otREG && _disInfo.OpType[1] == otIMM && (_disInfo.Immediate & 0x80000000) != 0) {
		...
		if(_dd == 'snj') {
			...
			if(_op == OP_DEC && _disInfo.OpType[0] == otREG && _disInfo.OpRegIdx[0] == _regIdx) {
				...
				_instrLen = Disasm.Disassemble(Code + _curPos, (__int64)_curAdr, &_disInfo, 0);
				_op = Disasm.GetOp(_disInfo.Mnem);
				if(_op == OP_OR && _disInfo.OpType[0] == otREG && _disInfo.OpType[1] == otIMM && _disInfo.OpRegIdx[0] == _regIdx &&
...
int __fastcall IsCopyDynArrayToStack(DWORD fromAdr)
{
	...
	if(_op == OP_TEST && _disInfo.OpType[0] == otREG && _disInfo.OpType[1] == otREG && _disInfo.OpRegIdx[0] == _disInfo.OpRegIdx[1]) {
		...
		if(_dd == 'sj') {
			...
			_instrLen = Disasm.Disassemble(Code + _curPos, (__int64)_curAdr, &_disInfo, 0);
			_op = Disasm.GetOp(_disInfo.Mnem);
			if(_op == OP_MOV && _disInfo.OpType[0] == otREG && _disInfo.OpType[1] == otMEM) {
				_reg2Idx = _disInfo.OpRegIdx[0];
				_curPos += _instrLen;
				_curAdr += _instrLen;
				_instrLen = Disasm.Disassemble(Code + _curPos, (__int64)_curAdr, &_disInfo, 0);
				_op = Disasm.GetOp(_disInfo.Mnem);
				if(_op == OP_DEC && _disInfo.OpType[0] == otREG && _disInfo.OpRegIdx[0] == _reg1Idx) {
					_curPos += _instrLen;
					_curAdr += _instrLen;
					_instrLen = Disasm.Disassemble(Code + _curPos, (__int64)_curAdr, &_disInfo, 0);
					_op = Disasm.GetOp(_disInfo.Mnem);
					if(_op == OP_PUSH && _disInfo.OpType[0] == otREG && _disInfo.OpRegIdx[0] == _reg2Idx) {
						_curPos += _instrLen;
						_curAdr += _instrLen;
						_instrLen = Disasm.Disassemble(Code + _curPos, (__int64)_curAdr, &_disInfo, 0);
						_dd = *((DWORD*)_disInfo.Mnem);
						if(_dd == 'snj' && _disInfo.Immediate == _adr2) {
							...
							_instrLen = Disasm.Disassemble(Code + _curPos, (__int64)_curAdr, &_disInfo, 0);
							_op = Disasm.GetOp(_disInfo.Mnem);
							if(_adr1 == _curAdr && _op == OP_MOV && _disInfo.OpType[0] == otREG && _disInfo.OpType[1] == otREG &&
...
int __fastcall IsInt64Shr(DWORD fromAdr)
{
	...
	for(n = 1; n <= 2; n++) {
		...
		if(n == 1) {
			...
			if(!(_op == OP_SHR && _disInfo.OpNum == 2 && _disInfo.OpType[0] == otREG && _disInfo.OpType[1] == otIMM && _disInfo.OpRegIdx[0] == _idx &&
				 _disInfo.Immediate == _val))
				break;
			return _curAdr + _instrLen - fromAdr;
		}
		if(_disInfo.Ret)
...
int __fastcall IsInt64Shl(DWORD fromAdr)
{
	...
	for(n = 1; n <= 2; n++) {
		...
		if(n == 1) {
			...
			if(!(_op == OP_SHL && _disInfo.OpNum == 2 && _disInfo.OpType[0] == otREG && _disInfo.OpType[2] == otIMM && _disInfo.OpRegIdx[0] == _idx &&
				 _disInfo.Immediate == _val))
				break;
			return _curAdr + _instrLen - fromAdr;
		}
...
{
	if(line.Pos("Enum_") || line.Pos("Array_") || line.Pos("DynArray_") || line.Pos("Set_") || line.Pos("Record_") || line.Pos("Unknown_"))
		return true;
	return false;
}
//---------------------------------------------------------------------------
