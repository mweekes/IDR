	...
	if(loadExp) {
		...
		if(ExpRVA) {
			...
			for(i = 0; i < ExpNum; i++) {
				...
				recE->address =
				  *((DWORD*)(this->FPEHeader32.Image + Adr2Pos(ExpFuncAdrPos + 4 * dw + this->FPEHeader32.ImageBase))) + this->FPEHeader32.ImageBase;
			...
			while(1) {
				...
				while(1) {
					...
					if(ThunkValue & 0x80000000) {
						...
						NameLength = strlen((char*)(this->FPEHeader32.Image + Adr2Pos(ThunkValue + 2 + this->FPEHeader32.ImageBase)));
