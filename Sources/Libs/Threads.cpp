...
void __fastcall TAnalyzeThread::FindVMTs2()
{
	...
	for(i = 0; i < CodeSize && !Terminated; i += 4) {
		...
		if(i + StopAt - classVMT - cVmtSelfPtr >= CodeSize)
			continue;
...
StdUnitInfo StdUnits[] = { { false, "Types", 0.0, -1 },    { false, "Multimon", 0.0, -1 }, { false, "VarUtils", 0.0, -1 }, { false, "StrUtils", 0.0, -1 },
						   { false, "Registry", 0.0, -1 }, { false, "IniFiles", 0.0, -1 }, { false, "Clipbrd", 0.0, -1 },  { false, 0, 0.0, -1 } };
...
void __fastcall TAnalyzeThread::ScanCode()
{
	...
	for(n = 0; n < (int)TotalSize && !Terminated; n++) {
		if((n & stepMask) == 0)
			UpdateProgress();
		recN = GetInfoRec(Pos2Adr(n));
		if(recN && recN->kind == ikVMT) {
			...
			if(!recU->names->Count) {
				...
				for(m = fromPos; m < toPos && !Terminated; m++) {
					if(IsFlagSet(cfProcStart, m)) {
						if(r != 5 && (int)Pos2Adr(m) == recU->iniadr)
							continue;
						if(r != 5 && (int)Pos2Adr(m) == recU->finadr)
							continue;
