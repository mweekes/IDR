...
char* RegCombTab[8] = { "bx+si", "bx+di", "bp+si", "bp+di", "si", "di", "bp", "bx" };
char* RepPrefixTab[4] = { "lock", "repne", "repe", "rep" };
//---------------------------------------------------------------------------
...
void __fastcall MDisasm::FormatInstr(PDISINFO pDisInfo, char* disLine)
{
	...
	if(!GetAddressSize() && GetCop() == 0xE3)
		OpName = "jcxz";

	if(disLine)
		strcat(disLine, OpName);
